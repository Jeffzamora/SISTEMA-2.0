<?php
    ...
    switch($_GET["op"]){
        ...
        case "mostrar";
            ...
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    ...
                    $output["not_mensaje"] = $row["not_mensaje"] . ' ' . $row["remi_id"];
        ...
        case "listar":
            ...
            foreach($datos as $row){
                $sub_array = array();
                $sub_array[] = $row["not_mensaje"] . ' ' . $row["remi_id"];
                $sub_array[] = '<button type="button" onClick="ver('.$row["remi_id"].');"  id="'.$row["remi_id"].'" class="btn btn-inline btn-info btn-sm ladda-button"><i class="fa fa-eye"></i></button>';
                $data[] = $sub_array;
            }
